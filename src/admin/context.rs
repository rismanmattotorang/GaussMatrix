...
pub(crate) struct Context<'a>(&'a gaussmatrix_service::admin::Context<'a>);

impl<'a> Context<'a> {
	#[inline]
	pub(crate) const fn new(inner: &'a gaussmatrix_service::admin::Context<'a>) -> Self {
		Self(inner)
	}
}

impl<'a> Deref for Context<'a> {
	type Target = gaussmatrix_service::admin::Context<'a>;
