...
#[config_example_generator(
	filename = "gaussmatrix-example.toml",
	section = "global",
	undocumented = "# This item is undocumented. Please contribute documentation for it.",
	header = r#"### GaussMatrix Configuration
###
### GaussMatrix — a sovereign, agentic-AI-ready Matrix homeserver in Rust by
### Gaussian Technologies. Built on the Tuwunel codebase (Apache-2.0); see
...
### https://gaussmatrix.dev/configuration.html
"#,
...
pub struct Config {
	...
	/// EXPERIMENTAL: back the content-addressed media store (CAS) with a shared
	/// object-store provider instead of the node-local gm-store column family.
	///
	/// Set to the name of an entry in `media_storage_providers` (e.g. an S3
	/// bucket). When set, CAS blobs are written to and read from that provider
	/// under a `media_cas/` prefix, keyed by content hash — so multiple nodes
	/// pointing at the same backend share one deduplicated blob namespace with
	/// no write conflicts (content-addressing makes every write idempotent).
	/// When empty (default), CAS uses the local gm-store backend (single node).
	///
	/// Only takes effect together with `media_cas_backend = true`.
	///
	/// reloadable: yes
	/// default: ""
	#[serde(default)]
	pub media_cas_provider: Option<String>,
...
#[config_example_generator(filename = "gaussmatrix-example.toml", section = "global.tls")]
...
#[config_example_generator(filename = "gaussmatrix-example.toml", section = "global.ldap")]
...
#[derive(Clone, Debug, Default, Deserialize)]
#[config_example_generator(filename = "gaussmatrix-example.toml", section = "global.jwt")]
