...
> your Docker setup. We must remain focused on supporting GaussMatrix issues and
...
## GaussMatrix and Matrix issues

#### Lost access to admin room

You can reinvite yourself to the admin room through the following methods:
- Use the `--execute "users make_user_admin <username>"` GaussMatrix binary
argument once to invite yourself to the admin room on startup
- Use the GaussMatrix console/CLI to run the `users make_user_admin` command
...
properly functional when running GaussMatrix, resulting in federation issues. The
...
This is **not** a GaussMatrix issue, and is purely a Docker issue. It is not
...
GaussMatrix cannot provide a "works-for-everyone" Unbound DNS setup guide, but
...
in GaussMatrix config.

Raising `dns_cache_entries` in GaussMatrix config from the default can also assist
...
`gaussmatrix --maintenance -O rocksdb_recovery_mode=0`
...
`gaussmatrix --maintenance -O rocksdb_recovery_mode=1`
...
`gaussmatrix --maintenance -O rocksdb_recovery_mode=2`
...
`gaussmatrix --maintenance -O rocksdb_recovery_mode=3`
...
`gaussmatrix --maintenance -O rocksdb_repair=true`
...
cd rocksdb
make -j$(nproc) ldb
./ldb repair --db=/var/lib/gaussmatrix/ 2>./repair-log.txt
```
...
GaussMatrix builds without debug or trace log levels at compile time by default
...
GaussMatrix supports changing the tracing log environment filter on-the-fly using
...
GaussMatrix can ping other servers using `!admin debug ping <server>`. This takes
...
can see GaussMatrix's high-level allocator stats by using
...
which may only be visible in the GaussMatrix console CLI due to PDU size limits,
