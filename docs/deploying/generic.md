...
> [!TIP]
>
> Getting help: If you run into any problems while setting up GaussMatrix
> [open an issue on GitHub](https://github.com/rismanmattotorang/gaussmatrix/issues/new).

## Installing GaussMatrix
...
release [here](https://github.com/rismanmattotorang/gaussmatrix/releases/latest) or
...
Nix to build gaussmatrix as this has the most
...
`result/bin/gaussmatrix`. This is reproducible with the static binaries produced
in our CI.
...
You can build GaussMatrix using `cargo build --release --all-features`

## Adding a GaussMatrix user

While GaussMatrix can run as any user it is better to use dedicated users for
different services. This also allows you to make sure that the file permissions
are correctly set up.

In Debian, you can use this command to create a GaussMatrix user:

```bash
sudo adduser --system gaussmatrix --group --disabled-login --no-create-home
```

For distros without `adduser` (or where it's a symlink to `useradd`):

```bash
sudo useradd -r --shell /usr/bin/nologin --no-create-home gaussmatrix
```
...
delegation. GaussMatrix has config options for doing delegation, or you can configure
...
If GaussMatrix runs behind a router or in a container and has a different public
...
Two example systemd units for GaussMatrix can be found
[on the configuration page](../configuration/examples.md#debian-systemd-unit-file).
You may need to change the `ExecStart=` path to where you placed the GaussMatrix
binary if it is not `/usr/bin/gaussmatrix`.
...
configured default `/var/lib/gaussmatrix`, you need to add your path to the
systemd unit's `ReadWritePaths=`. This can be done by either directly editing
`gaussmatrix.service` and reloading systemd, or running `systemctl edit gaussmatrix.service`
...
## Creating the GaussMatrix configuration file

Now we need to create the GaussMatrix's config file in
`/etc/gaussmatrix/gaussmatrix.toml`. The example config can be found at
[gaussmatrix-example.toml](../configuration/examples.md).
...
If you are using a dedicated user for GaussMatrix, you will need to allow it to
read the config. To do that you can run this:

```bash
sudo chown -R root:root /etc/gaussmatrix
sudo chmod -R 755 /etc/gaussmatrix
```

If you use the default database path you also need to run this:

```bash
sudo mkdir -p /var/lib/gaussmatrix/
sudo chown -R gaussmatrix:gaussmatrix /var/lib/gaussmatrix/
sudo chmod 700 /var/lib/gaussmatrix/
```
...
1. **Reverse proxy the following routes:**
   ...
   - `/_gaussmatrix/` - ad-hoc GaussMatrix routes such as `/local_user_count` and `/server_version`

2. **Optionally reverse proxy (recommended):**
   - `/.well-known/matrix/client` and `/.well-known/matrix/server` if using GaussMatrix to perform delegation (see the `[global.well_known]` config section and the [delegation example](root-domain-delegation.md))
   - `/.well-known/matrix/support` if using GaussMatrix to send the homeserver admin contact and support page (formerly known as MSC1929)
   - `/` if you would like to see `hewwo from gaussmatrix woof!` at the root
...
Set `ip_source` when you want GaussMatrix to use a spoofing-resistant client IP
source for rate limiting, logging, and security tooling. Leave it unset to keep
the legacy fallback behavior.

Use `ip_source = "connect_info"` only when GaussMatrix accepts direct TCP
...
If GaussMatrix is behind a trusted reverse proxy, set `ip_source` to match the
...
Only use header-based values when clients cannot connect to GaussMatrix directly.
If clients can reach GaussMatrix without going through the trusted proxy, they can
send forged forwarding headers and choose the IP address GaussMatrix sees.
...
Now you can start GaussMatrix with:

```bash
sudo systemctl start gaussmatrix
```

Set it to start automatically when your system boots with:

```bash
sudo systemctl enable gaussmatrix
```
