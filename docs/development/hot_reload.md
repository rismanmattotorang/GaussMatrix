...
When developing in debug-builds with the nightly toolchain, GaussMatrix is modular
...
linkers, and please let us know in the [GaussMatrix Matrix room][7] the linker
...
0. Have patience. Don't hesitate to join the [GaussMatrix Matrix room][7] to
...
1. Install the nightly toolchain using rustup. You may need to use `rustup
   override set nightly` in your local GaussMatrix directory, or use `cargo
...
9. In your GaussMatrix server terminal, hit/send `CTRL+C` signal. This will tell
   GaussMatrix to find which libraries need to be reloaded, and reloads them as
   necessary.
...
To shutdown GaussMatrix in this setup, hit/send `CTRL+\`. Normal builds still
...
the [GaussMatrix Matrix room][7] the problem. mold can be installed typically
...
![GaussMatrix's dynamic library setup diagram - created by Jason
...
![GaussMatrix's reload and load order diagram - created by Jason
...
[7]: https://matrix.to/#/#gaussmatrix:grin.hu
